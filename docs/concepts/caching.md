...
- **Neither is pricing.** `cost_usd` is recomputed on every hit from the stored token counts and the current rate sheet, so editing `pricing:` re-costs a warm suite without re-running it. That cuts both ways: upgrading to a domarinn whose built-in table has gained your model prices the suite you already have, and a run that reported no cost at all can start reporting one without a single new provider call. Keying pricing would discard every entry the day a vendor changed a price. (`exec` children report their own cost, which domarinn cannot re-derive, so theirs is replayed as stored.)
...
Declaring it is the whole guard, and it holds against a store carried across versions too: a provider with a non-empty `env` is not offered the [pre-0.3.1 key shapes](#upgrading-to-05), which have no `env` member and would therefore match whatever the variable is set to.
...
- **A provider that declares `env` does not adopt the oldest two shapes.** `env` only joined the exec key in 0.3.1, so an entry written by 0.3.0 or earlier records no environment at all — matching one would mean every declared value resolving to the same entry, and a suite re-pointed at a different backend replaying the old backend's answers. Those two generations are therefore withheld from a provider with a non-empty `env` and offered only to one that declares none, which is the case they exist for. The cost is a re-run of cells whose only ancestor is that old; the alternative is a comparison that was never made.
