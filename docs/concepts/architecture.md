...
**The result schema version is the CLI↔server↔UI contract.** `RunResult` — what a run produces — carries `RESULT_SCHEMA_VERSION` (`3` as of this writing). The server's `/api/v1/meta` reports both that number and the window it will still ingest (`result_schema_version - 1 ..= result_schema_version`), so a CLI one release behind can still upload, and a body outside the window is rejected with `422` rather than silently misread — with a message that names which side of the window was crossed, and therefore which binary to upgrade. The web UI's TypeScript types are generated from the same Rust structs the CLI serializes, so the three components can be built and released independently without inventing a second source of truth for what a run document looks like.
