...
#### Tool-using transcripts

A turn is not only prose. An `assistant` turn may carry `tool_calls`, and a `tool` turn carries the result that came back — which is what makes `history` usable for the case it matters most for: an agent told to call something first will, in a single-turn eval, call it and stop, because nothing feeds a result back. From the shipped [example 42](../examples/templates-and-test-data.md#example-42--replaying-a-tool-using-transcript):

```yaml
--8<-- "examples/42-tool-call-history/domarinn.yaml:tools"
```

`tool_calls` is field-for-field the shape a provider **reports** on the way out, so the `tool_calls` block of a stored case pastes into a suite unchanged. `tool_call_id` is optional: when a transcript omits ids, a result is paired with the call at the same position in the preceding round, so a parallel round needs no ids at all. A call's `name` and `id` are **not** templated (a call must be spelled like the `tools:` entry it names); its `arguments` **are**, leaf by leaf, so each leaf keeps its own type — an untemplated `{order_id: 1042}` stays an integer rather than becoming `"1042"`. A leaf you *do* template renders to a **string** (`{order_id: "{{ n }}"}` sends `"7"`, not `7`), and that applies to a whole-value reference too — templating is string-in, string-out at every leaf. So a non-string argument has to be written literally in the suite; varying one per case is not expressible today. A case that needs different *typed* arguments needs a case per shape.

A turn's `content` may also be a list of typed blocks instead of a string — `{type: text, text: …}` and `{type: thinking, thinking: …, signature: …}`. A `thinking` block is **never templated and never `file://`-loaded**: its `signature` is a vendor integrity token over those exact bytes, so any rewrite invalidates the replay. Blocks are otherwise ordinary content; `content: "hi"` remains the spelling for the overwhelmingly common case and serializes identically to before blocks existed.

**Provider notes.** Role ordering is deliberately not *validated* — it is the provider's contract, and `validate` will never **refuse** a suite over it. Three shapes do draw a **warning** (never an error, never a changed exit code): a history whose first non-`system` turn is `assistant`, or is `tool` — both only *when the suite splices history at the front of the transcript*, where a `tool` turn answers a call nothing made — and a turn whose `content` is empty. An Anthropic assistant **prefill** is exactly the first shape, which is why it is advice rather than a rule. The reason domarinn says anything at all: at run time these surface as an errored cell, which reads like an outage rather than a typo — and an errored cell is never cached, so it is re-paid on every run. One shape is an *error* rather than advice: a turn carrying neither `content` nor `tool_calls` is rejected, because no provider can read it. The warnings only see what is in the file: a `history: file://…` transcript is not read at validate time, and content that only becomes empty after templating is not predicted.

The `anthropic` provider hoists any `system` turns (history's included) into its top-level `system` field, and folds a round of consecutive `tool` turns into **one** `user` message of `tool_result` blocks; `openai` receives the array with the mirror-image treatment — one `role: "tool"` message per result, and `function.arguments` re-encoded as the JSON string that vendor expects — and has no `thinking` concept, so those blocks are dropped there rather than leaked into the conversation as prose. `exec` receives the JSON verbatim. The `http` provider flattens the transcript's prose into the `{{ prompt }}` string, which deliberately does **not** render tool calls; a tool-bearing transcript is visible structurally as [`{{ messages }}`](providers.md#http). The [LLM-rubric grader](#grader) builds its own single-turn requests and does not see case history.
...
A suite is validated structurally before any provider is contacted — both by `domarinn validate` and at the start of `domarinn run`. Validation is strict on purpose: a typo is an error you can fix, not a field that is silently ignored while the setting it was meant to change quietly does nothing. Strictness is about **unknown fields** — a key that does nothing is an error. A warning is the other axis: a *known* field holding a value that is almost certainly a mistake but might not be. It is printed, and the suite still runs.
