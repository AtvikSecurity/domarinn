...
`tool_calls` is field-for-field the shape a provider **reports** on the way out, so the `tool_calls` block of a stored case pastes into a suite unchanged. `tool_call_id` is optional: when a transcript omits ids, a result is paired with the call at the same position in the preceding round, so a parallel round needs no ids at all. A call's `name` and `id` are **not** templated (a call must be spelled like the `tools:` entry it names); its `arguments` **are**, leaf by leaf, so each leaf keeps its own type — an untemplated `{order_id: 1042}` stays an integer rather than becoming `"1042"`. A leaf you *do* template renders to a string (`{order_id: "{{ n }}"}` sends `"7"`, not `7`); to vary a numeric argument per case, put the whole `arguments` object in a var, which passes through untouched.
