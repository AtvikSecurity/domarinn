...
`tool_calls` is field-for-field the shape a provider **reports** on the way out, so the `tool_calls` block of a stored case pastes into a suite unchanged. `tool_call_id` is optional: when a transcript omits ids, a result is paired with the call at the same position in the preceding round, so a parallel round needs no ids at all. A call's `name` and `id` are **not** templated (a call must be spelled like the `tools:` entry it names); its `arguments` **are**, leaf by leaf, so `{order_id: 1042}` stays an integer rather than becoming `"1042"`.
