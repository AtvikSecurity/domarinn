...
- A grader is a provider like any other, so the same redirect moves the **judge**.
...
The system under test in that suite is the offline echo provider, so the **only** thing reaching a network is the judge. That is the shape to reach for when you want to develop a rubric: the answers are fixed, the verdict is the variable, and each attempt is free.
...
### A local judge is a different judge
...
A frontier judge scores that case 1.00. Neither verdict is a bug: a rubric is calibrated **against a judge**, and a smaller model reads instructions more literally and volunteers more opinions. So use a local judge for the loop you iterate in, and re-calibrate — or re-baseline — when you change the model that grades. Note also the `COST –` and the `100% cached` on this run: verdicts are cached per judge identity, which brings us to the caveat below.
...
| [33 — an OpenAI-shaped judge](../examples/models-grading-and-budgets.md#example-33--an-openai-shaped-judge) | yes | The grader is `type: openai`, so the judge moves with the same variable. |
...
| [29 — LLM-rubric grading](../examples/models-grading-and-budgets.md#example-29--llm-rubric-grading) | no | Its grader is `type: anthropic`. Use **33** instead — same rubric, OpenAI-shaped judge. |
...
| Judge | a larger `qwen3` | Verdicts are where model size shows. A 4B judge works and is what CI uses, but expect it to be stricter and noisier than a frontier one. |
...
- Switch `OPENAI_MODEL` and the same thing happens, including for the grader — **judge identity is in the key**, so grading a case with `gpt-4o-mini` and then with `qwen3:4b` asks the second judge rather than replaying the first one's verdict.
...
- [Example 26](../examples/models-grading-and-budgets.md#example-26--an-openai-compatible-endpoint) and [example 33](../examples/models-grading-and-budgets.md#example-33--an-openai-shaped-judge) — the suites above.
