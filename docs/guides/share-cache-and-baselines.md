...
**Uploading a run is the other way round.** The cache degrades because a cold cache still answers the question, only slower and dearer. `domarinn run --share` has no such fallback — the results have exactly one destination — so a failed upload **fails the run with exit `3`**, and a server whose result-schema window excludes the CLI — or no server URL configured at all — is refused with exit `2` before the suite spends anything. Pass `--allow-share-failure` (the action's `allow-share-failure` input) where publishing is genuinely optional. See [Uploading CI runs](gate-in-ci.md#uploading-ci-runs-to-a-shared-server).
