...
Three details. Arguments are templated leaf by leaf, so each leaf keeps its own type: an untemplated `{order_id: 1042}` stays an integer, which is what a [`tool-call` assertion](../reference/assertions.md#tool-call) comparing against the decoded object needs. The corollary is easy to trip over — a leaf you *do* template comes out a string, so a numeric argument that varies per case belongs in a var holding the whole `arguments` object. A turn's `content` may be a list of typed blocks instead of a string, which is how `thinking` is replayed; that one is **never** templated, because its `signature` is a vendor integrity token over those exact bytes. And the two vendors disagree in mirrored ways about a round of parallel results — `anthropic` folds them into one user message, `openai` sends one message each — which domarinn handles so the suite does not have to say.
