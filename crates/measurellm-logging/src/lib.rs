//! Shared tracing-subscriber setup for measurellm binaries.
...
pub fn try_init(opts: &LogOptions) -> Result<(), tracing_subscriber::util::TryInitError> {
    ...
    let env_format = std::env::var("MEASURELLM_LOG_FORMAT").ok();
...
fn default_directives(profile: LogProfile, verbose: u8) -> String {
    ...
    // `measurellm={level}` prefix-matches every `measurellm_*` crate target:
    // EnvFilter directives match targets by prefix, so this one directive
    // covers measurellm_core, measurellm_cache, measurellm_server, etc.
    match profile {
        LogProfile::Cli => format!("measurellm={level}"),
        LogProfile::Server => format!("measurellm={level},tower_http={level}"),
    }
}

/// Resolve the effective [`LogFormat`], given an explicit flag value, the
/// raw `MEASURELLM_LOG_FORMAT` env value (if any), the active profile, and
...
fn resolve_format(
    ...
    if let Some(value) = env_value {
        match value {
            ...
            other => eprintln!(
                "measurellm-logging: ignoring invalid MEASURELLM_LOG_FORMAT value {other:?}; autodetecting instead"
            ),
        }
    }
...
mod tests {
    ...
    fn cli_directives_by_verbosity() {
        assert_eq!(default_directives(LogProfile::Cli, 0), "measurellm=warn");
        assert_eq!(default_directives(LogProfile::Cli, 1), "measurellm=info");
        assert_eq!(default_directives(LogProfile::Cli, 2), "measurellm=debug");
        assert_eq!(default_directives(LogProfile::Cli, 3), "measurellm=trace");
    }

    #[test]
    fn cli_directives_clamp_at_trace() {
        // verbose way past the table should still clamp to trace, not panic.
        assert_eq!(default_directives(LogProfile::Cli, 10), "measurellm=trace");
    }
    ...
    fn server_directives_include_tower_http() {
        assert_eq!(
            ...
            "measurellm=info,tower_http=info"
        );
    }
    ...
    fn server_directives_reach_trace_at_verbose_2() {
        ...
        assert_eq!(
            ...
            "measurellm=trace,tower_http=trace"
        );
    }
