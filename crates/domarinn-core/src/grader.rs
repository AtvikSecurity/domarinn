...
impl DefaultGrader {
    ...
    async fn grade_llm_rubric(
        ...
        let verdict = match &grader.provider {
            ...
            other => Err(format!(
                "grader provider type {other:?} is not supported for llm-rubric"
            )),
        };
    ...
    async fn anthropic_verdict(
        ...
        let key = api_key(api_key_env.unwrap_or("ANTHROPIC_API_KEY")).map_err(|e| e.to_string())?;
        let base = base_url
        ...
        if payload.get("stop_reason").and_then(|s| s.as_str()) == Some("max_tokens") {
            return Err(
                "verdict truncated (stop_reason=max_tokens); raise grader max_tokens".into(),
            );
        }
        let input = payload
            .get("content")
            ...
            .ok_or("no tool_use verdict in response")?;
    ...
    async fn openai_verdict(
        ...
        let key = api_key(api_key_env.unwrap_or("OPENAI_API_KEY")).map_err(|e| e.to_string())?;
        let base = base_url
        ...
        if choice
            ...
            == Some("length")
        {
            return Err("verdict truncated (finish_reason=length); raise grader max_tokens".into());
        }
        let content = choice
            .and_then(|c| c.get("message"))
            .and_then(|m| m.get("content"))
            .and_then(|c| c.as_str())
            .ok_or("no verdict content in response")?;
        let value: Json =
            serde_json::from_str(content).map_err(|e| format!("verdict not JSON: {e}"))?;
        Verdict::from_json(&value)
    }
}

/// The reject-list for grader params incompatible with forced tool use.
fn reject_thinking(params: Option<&crate::config::ParamMap>) -> Result<(), String> {
    if let Some(p) = params {
        if p.contains_key("thinking") || p.contains_key("reasoning") {
            return Err(
                "grader params must not enable extended thinking: forced tool use is rejected when \
                 thinking is on. Remove `thinking`/`reasoning`."
                    .into(),
            );
        }
    }
...
impl Verdict {
    fn from_json(v: &Json) -> Result<Verdict, String> {
        let pass = v
            ...
            .ok_or("verdict missing `pass`")?;
        let score = v
            .get("score")
            .and_then(|s| s.as_f64())
            .ok_or("verdict missing `score`")?
            .clamp(0.0, 1.0);
        let reasoning = v
            .get("reasoning")
            .and_then(|r| r.as_str())
