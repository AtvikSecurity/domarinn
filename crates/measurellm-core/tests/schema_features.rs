...
fn schema_text() -> String {
    serde_json::to_string(&measurellm_core::config_schema()).unwrap()
}
...
fn result_schema_is_versioned() {
    let schema = serde_json::to_string(&measurellm_core::result_schema()).unwrap();
    assert!(schema.contains("schema_version"));
    assert_eq!(measurellm_core::RESULT_SCHEMA_VERSION, 1);
}

#[test]
fn exports_typescript_definitions() {
    let dir = tempfile::tempdir().unwrap();
    measurellm_core::export_types(dir.path()).unwrap();
