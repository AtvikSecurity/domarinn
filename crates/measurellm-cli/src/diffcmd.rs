...
use clap::Args;
use measurellm_core::diff::{diff_runs, Delta, RunDiff};
...
pub fn render_markdown(diff: &RunDiff) -> String {
    ...
    out.push_str(&format!("### measurellm comparison — {verdict}\n\n"));
...
pub fn write_summary_md(
    ...
    let mut out = String::new();
    out.push_str(&format!(
        "### measurellm run — {} passed, {} failed, {} errored\n\n",
        s.passed, s.failed, s.errored
    ));
    let rate = measurellm_core::stats::wilson(s.passed, s.total, measurellm_core::stats::Z_95);
    out.push_str(&format!(
