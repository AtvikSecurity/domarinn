//! `measurellm import promptfoo <PATH>`: translate a promptfoo config into a
//! measurellm suite. Mappable constructs are converted; anything without a
...
pub fn execute(path: PathBuf) -> u8 {
    ...
    match serde_yaml_ng::to_string(&suite) {
        Ok(yaml) => {
            println!("# Converted from a promptfoo config by `measurellm import promptfoo`.");
            println!("# yaml-language-server: $schema=./measurellm.schema.json");
            for note in &notes {
                println!("# NOTE: {note}");
            }
            print!("{yaml}");
...
/// `anthropic:messages:claude-3-5-sonnet` to a measurellm provider.
