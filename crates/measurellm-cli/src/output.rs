...
use measurellm_core::result::{CaseStatus, RunResult};
...
fn display_name(case: &measurellm_core::result::CaseResult) -> String {
    case.name
        .clone()
...
fn render_junit(result: &RunResult) -> String {
    ...
    out.push_str(&format!(
        ...
        xml_escape(result.suite.as_deref().unwrap_or("measurellm")),
        s.total,
        s.failed,
        s.errored,
    ));
