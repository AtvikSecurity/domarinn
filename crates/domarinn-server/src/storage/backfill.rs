//! One-shot, idempotent backfill of the migration-3 columns from the stored
//! zstd blobs.
//!
//! Migration 3 adds `provider_id`/`prompt_id`/`test_id`/`repeat_idx`/`score`/
//! `stop_reason` to `cases` and `config_digest` to `runs`, but a schema
...
//! NULL` / `config_digest IS NULL`, so a fully-backfilled database (and every
//! fresh database, whose rows are written already-populated by ingest) selects
//! zero rows and the loops exit immediately. A row whose blob cannot be
//! decompressed or parsed is stamped with the empty-string sentinel so it is
//! never rescanned — the alternative (leaving it NULL) would spin forever.
...
fn backfill_cases(conn: &mut Connection) -> anyhow::Result<()> {
    loop {
        ...
        let chunk: Vec<(i64, Option<Vec<u8>>)> = {
            let mut stmt =
                conn.prepare("SELECT rowid, detail FROM cases WHERE provider_id IS NULL LIMIT ?1")?;
            ...
            match decode_case(detail.as_deref()) {
                    ...
                    tx.execute(
                        "UPDATE cases
                         SET provider_id = ?1, prompt_id = ?2, test_id = ?3,
                             repeat_idx = ?4, score = ?5, stop_reason = ?6
                         WHERE rowid = ?7",
                        params![
                ...
                Err(error) => {
                    tracing::warn!(
                        rowid,
                        error = %error,
                        "backfill: undecodable case detail; stamping empty-string sentinel"
                    );
                    tx.execute(
                        "UPDATE cases SET provider_id = '' WHERE rowid = ?1",
                        params![rowid],
                    )?;
                }
            }
...
fn backfill_runs(conn: &mut Connection) -> anyhow::Result<()> {
    loop {
        let chunk: Vec<(String, Vec<u8>)> = {
            let mut stmt = conn.prepare(
                "SELECT run_id, body FROM run_blobs
                 WHERE run_id IN (SELECT id FROM runs WHERE config_digest IS NULL)
                 LIMIT ?1",
            )?;
        ...
        for (run_id, body) in chunk {
            match decode_config_digest(&body) {
                Ok(digest) => {
                    tx.execute(
                        "UPDATE runs SET config_digest = ?1 WHERE id = ?2",
                        params![digest, run_id],
                    )?;
                }
                Err(error) => {
                    tracing::warn!(
                        run_id = %run_id,
                        error = %error,
                        "backfill: undecodable run blob; stamping empty-string sentinel"
                    );
                    tx.execute(
                        "UPDATE runs SET config_digest = '' WHERE id = ?1",
                        params![run_id],
                    )?;
                }
            }
...
/// Pull just `config_digest` out of a `run_blobs.body` blob without fully
/// deserializing the (large) `RunResult`.
fn decode_config_digest(body: &[u8]) -> anyhow::Result<String> {
