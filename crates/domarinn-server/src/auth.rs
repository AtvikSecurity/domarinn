...
/// Prefix for API-key secrets (`domarinn_<hex>`).
...
/// (`header.payload.signature`) whereas the static `domarinn_` tokens never do, so a
...
impl StaticTokenAuthenticator {
    ...
    /// Parse a `DOMARINN_TOKENS` value: comma-separated `scope:token` pairs.
...
pub enum IdentitySource {
    Anonymous,
    /// A configured `DOMARINN_TOKENS` static token.
    Static,
    /// A local-account API key (`domarinn_...`).
    ApiKey,
...
mod tests {
    ...
    fn parses_token_env() {
        let auth = StaticTokenAuthenticator::from_env_value(
            "write:domarinn_ci, admin:domarinn_ops,read:domarinn_view",
        );
        assert!(auth.has_tokens());
        assert_eq!(
            auth.authenticate("domarinn_ci").unwrap().scope,
            Scope::Write
        );
        assert_eq!(
            auth.authenticate("domarinn_ops").unwrap().scope,
            Scope::Admin
        );
        assert_eq!(
            auth.authenticate("domarinn_view").unwrap().scope,
            Scope::Read
        );
    ...
    fn token_formats_and_prefix() {
        ...
        assert!(key.starts_with("domarinn_"));
        assert_eq!(key.len(), "domarinn_".len() + 64);
